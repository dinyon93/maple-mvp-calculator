(function () {
  ...
  async function load() {
    if (location.protocol === 'file:') { history = localRead(); $('trendStorageNote').textContent = '이 브라우저에 날짜별로 저장됩니다.'; }
    else {
      try { const response = await fetch('/api/trends'); if (!response.ok) throw Error('기록을 읽지 못했습니다.'); history = await response.json(); $('trendStorageNote').textContent = '로컬 프로그램의 data/trends.json에 저장됩니다.'; }
      catch (error) { $('trendStorageNote').textContent = `기록을 불러오지 못했습니다: ${error.message}`; }
    }
    render();
  }
  async function record(values) {
    ...
    if (location.protocol === 'file:') {
      ...
      try {
        const response = await fetch('/api/trends', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ values: clean }) });
        if (!response.ok) throw Error('날짜별 시세 저장 실패');
        history = await response.json();
      } catch (error) { $('trendStorageNote').textContent = error.message; }
    }
